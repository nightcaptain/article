...
//	}
	 ...
	 * @return int 转换完成的整型
	 * */
	public static Integer getStrToInteger(Object value)
	{
        Integer iValue = null;
		if (value!=null)
		{
	        try
	        {
	            iValue = Integer.parseInt(value.toString().trim());
	        }
	        catch (Exception ex)
	        {
	            ex.printStackTrace();
    ...
    public static Long getStrToLong(Object value)
    {
        Long iValue = null;
        if (value!=null)
        {
            try
            {
                iValue = Long.parseLong(value.toString().trim());
            }
            catch (Exception ex)
            {
                ex.printStackTrace();
...
//
    ...
    public static String subStr(String content,Integer length,String padding) throws UnsupportedEncodingException{
    	String str = "";
    	int paddSize =  org.apache.commons.lang3.StringUtils.isBlank(padding)? 0 : padding.length();
    	//如果内容为空，或者小于指定的长度。则返回原内容
    	if(org.apache.commons.lang3.StringUtils.isBlank(content) ||  content.length() <= length){
    		return content;
    	}
    	str = content.substring(0, length-paddSize);
    	if(org.apache.commons.lang3.StringUtils.isNotBlank(padding)){
    		str += padding;
    	}
    	return str;
    }
